  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 md:p-6">
      <div className="flex items-center justify-between mb-4 md:mb-6">
        <h2 className="text-lg md:text-2xl font-bold text-gray-800 dark:text-gray-100">{t('managePlayersTitle')}  </h2>
      ...
      {savedPlayers.length === 0 ? (
        ...
        <div className="space-y-2 md:space-y-3">
          <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 mb-3 md:mb-4">{savedPlayers.length} {savedPlayers.length === 1 ? t('savedPlayer') : t('savedPlayers')}</p>
          {savedPlayers.map((player, idx) => (
            <div key={idx} className="border border-gray-200 dark:border-gray-600 rounded-lg p-2 md:p-4 hover:border-indigo-300 dark:hover:border-indigo-500 transition bg-white dark:bg-gray-700">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
                  <PlayerAvatar player={player} size="md" className="flex-shrink-0" />
                  <p className="font-semibold text-gray-800 dark:text-gray-100 text-sm md:text-base truncate">{player.name}</p>
                </div>
                <button 
      ...
      <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-gray-200 dark:border-gray-600">
        <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">{t('managePlayersTip')}</p>
      </div>
    </div>
  );
};
